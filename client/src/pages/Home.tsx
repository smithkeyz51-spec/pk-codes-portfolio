...
const projects = [
  { name: "Waka", type: "Full-Stack Web Application", description: "A crowdsourced fare-tracking platform helping Nigerian commuters check real-world transport fares before boarding, with account creation, guest access, backend functionality, and an admin dashboard.", image: "/assets/waka-entry-phone.webp", tags: ["Full-Stack", "Web Application", "Backend", "Admin Dashboard", "Product Thinking", "Responsive Web"], url: "https://waka-app-eight.vercel.app/" },
  { name: "Beacon", type: "Web Application", description: "A browser-based find-my-phone tool that allows users to connect a device and view its location from another browser without requiring an app-store installation.", image: "/assets/beacon-authenticated-dashboard.webp", tags: ["Device workflow", "Browser-based", "Maps"], url: "https://beacon-wine-seven.vercel.app/" },
  { name: "Airville", type: "Client Website / Aviation", description: "A premium aircraft charter and logistics website designed to communicate luxury, trust and professionalism while presenting services clearly to potential clients.", image: "/assets/airville-real.webp", tags: ["Client work", "Aviation", "Brand web"], url: "https://airville-logistics-website.vercel.app/" },
  { name: "You're Enough", type: "Digital Product", description: "A safe-space digital product helping young people navigate pressure, purpose, emotions, and personal growth through practical tools and reflection.", image: "/assets/enough-real-product.webp", tags: ["UX focus", "Accessibility", "Interactive"], url: "https://youre-enoughv.vercel.app/", github: "https://github.com/smithkeyz51-spec/youre-enoughv" },
...
export default function Home() {
  ...
  return <div className={`site-shell${scrolled ? " has-scrolled" : ""}`} style={{ "--scroll-progress": `${scrollProgress * 100}%` } as CSSProperties}>
    ...
    <header className={`topbar${scrolled ? " scrolled" : ""}`}><a className="brand" href="/" aria-label="PK-CODES home"><img className="brand-logo" src={navbarLogo} alt="PK-CODES" /></a><nav className={menuOpen ? "nav-links open" : "nav-links"} aria-label="Primary navigation">{["About", "Skills", "Projects", "Experience", "Contact"].map((item) => <button key={item} onClick={() => scrollTo(item.toLowerCase())}>{item}</button>)}<a className="nav-cta" href="mailto:smithkeyz51@gmail.com">Contact Me <ArrowUpRight size={14} /></a></nav><button className="menu-toggle" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle navigation">{menuOpen ? <X /> : <Menu />}</button></header>
    <main>
