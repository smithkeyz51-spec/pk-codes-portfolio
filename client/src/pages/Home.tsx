...
const projects = [
  { name: "Waka", type: "Digital Product / Web Application", description: "A crowdsourced fare-tracking platform helping Nigerian commuters check real-world transport fares before boarding.", image: "/manus-storage/waka-real_55c4bc71.webp", tags: ["Product thinking", "Responsive web", "Vercel"], url: "https://waka-eight.vercel.app/" },
  { name: "Beacon", type: "Web Application", description: "A browser-based find-my-phone tool that allows users to connect a device and view its location from another browser without requiring an app-store installation.", image: "/manus-storage/beacon-authenticated-dashboard_cdede3d6.webp", tags: ["Device workflow", "Browser-based", "Maps"], url: "https://beacon-wine-seven.vercel.app/" },
  { name: "Airville", type: "Client Website / Aviation", description: "A premium aircraft charter and logistics website designed to communicate luxury, trust and professionalism while presenting services clearly to potential clients.", image: "/manus-storage/airville-real_e6dbbe7e.webp", tags: ["Client work", "Aviation", "Brand web"], url: "https://airville-logistics-website.vercel.app/" },
  { name: "You're Enough", type: "Digital Product", description: "A safe-space digital product helping young people navigate pressure, purpose, emotions, and personal growth through practical tools and reflection.", image: "/manus-storage/enough-real-product_7ac1085b.webp", tags: ["UX focus", "Accessibility", "Interactive"], url: "https://youre-enoughv.vercel.app/", github: "https://github.com/smithkeyz51-spec/youre-enoughv" },
...
const services = [
  ...
  { number: "03", title: "UI / UX", copy: "Creating clean, intuitive interfaces with a focus on usability." },
  { number: "04", title: "Creative Direction", copy: "Experience in branding, visual identity, digital content and creative direction." },
];
...
export default function Home() {
  ...
  return <div className={`site-shell${scrolled ? " has-scrolled" : ""}`} style={{ "--scroll-progress": `${scrollProgress * 100}%` } as CSSProperties}>
    ...
    <header className={`topbar${scrolled ? " scrolled" : ""}`}><button className="brand" onClick={() => scrollTo("home")} aria-label="Back to top"><span className="brand-mark"><img src={store.logo} alt="" /></span><span>PK CODES</span></button><nav className={menuOpen ? "nav-links open" : "nav-links"} aria-label="Primary navigation">{["About", "Skills", "Projects", "Experience", "Contact"].map((item) => <button key={item} onClick={() => scrollTo(item.toLowerCase())}>{item}</button>)}<a className="nav-cta" href="mailto:smithkeyz51@gmail.com">Contact Me <ArrowUpRight size={14} /></a></nav><button className="menu-toggle" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle navigation">{menuOpen ? <X /> : <Menu />}</button></header>
    <main>
      <section className="hero section-wrap" id="home" data-reveal><div className="hero-text"><div className="availability"><span className="status-dot" />Available for new projects</div><p className="eyebrow">SOFTWARE DEVELOPER <span>·</span> DIGITAL BUILDER</p><h1>Hi, I'm <em>Gideon.</em><br /><span>I build what matters.</span></h1><p className="hero-copy">I build modern websites, digital products and experiences that turn ideas into something people can actually use.</p><div className="hero-actions"><button className="button button-primary" onClick={() => scrollTo("projects")}>View My Work <ArrowUpRight size={16} /></button><a className="button button-ghost" href="mailto:smithkeyz51@gmail.com">Contact Me <ArrowUpRight size={16} /></a><a className="cv-link" href={store.resume} target="_blank" rel="noreferrer">Download CV <Download size={14} /></a></div><div className="hero-footnote"><span>01</span><Minus size={14} /> From problem to product, with intention.</div></div>
        <div className="hero-art"><div className="art-orbit orbit-one" /><div className="art-orbit orbit-two" /><div className="identity-stamp"><img src={store.logo} alt="PK Codes mark" /><span>PK / 026</span></div><div className="robot-frame"><div className="robot-scanline" /><img className="robot-visual" src={store.robot} alt="Futuristic PK Codes digital assistant robot" /><div className="robot-caption"><span>PK CODES / DIGITAL ASSISTANT</span><span>ONLINE</span></div></div><div className="hero-note"><Sparkles size={14} /> Useful ideas, real interfaces</div></div>
      </section>
      <section className="signal-strip section-wrap" id="skills" data-reveal><div className="strip-heading"><SectionLabel>TECHNOLOGICAL FOUNDATION</SectionLabel><p>The tools and skills I use to bring ideas to life.</p></div><div className="skill-rail">{skills.map((skill, index) => <span className="skill-pill" key={skill}><span className={`skill-icon icon-${index % 4}`} />{skill}</span>)}</div></section>
      <section className="about section-wrap" id="about" data-reveal><div className="section-intro"><SectionLabel>ABOUT THE BUILDER</SectionLabel><h2>More than a<br /><span>developer.</span></h2></div><div className="about-copy"><p className="lead">I'm Gideon, a builder and software developer passionate about creating digital products and solutions that solve real-world problems.</p><p>I'm developing my expertise in software engineering while building websites and digital experiences. My background also includes creative direction, branding, community engagement and sales.</p><div className="about-tags"><span>Software Development</span><span>Digital Products</span><span>Creative Direction</span><span>Product Thinking</span></div></div></section>
      <section className="services section-wrap" data-reveal><div className="services-heading"><SectionLabel>BEYOND JUST CODING</SectionLabel><p>I bring the problem, the product, and the pixels into the same conversation.</p></div><div className="service-grid">{services.map((service) => <article className="service-card" key={service.number}><span className="card-number">{service.number}</span><h3>{service.title}</h3><p>{service.copy}</p><ChevronRight size={18} className="card-arrow" /></article>)}</div></section>
      <section className="projects section-wrap" id="projects" data-reveal><div className="projects-heading"><div><SectionLabel>SELECTED WORKS</SectionLabel><h2>Featured <span>projects.</span></h2><p>Real websites and digital products shaped from problem, to interface, to something people can use.</p></div><a className="text-link" href="https://github.com/smithkeyz51-spec" target="_blank" rel="noreferrer">See more on GitHub <ArrowUpRight size={15} /></a></div><div className="project-grid">{projects.map((project, index) => <article className={`project-card project-${index + 1}`} key={project.name}><a href={project.url} target="_blank" rel="noreferrer" className={`project-image project-image-${index + 1} ${project.image ? "" : "project-image-text"}`}>{project.image ? <img src={project.image} alt={`${project.name} live website screenshot`} /> : <><span className="heralds-mark">THE<br />HERALDS</span><small>Authentic project reference unavailable online</small></>}<span className="project-view">View live <ExternalLink size={13} /></span></a><div className="project-meta"><span className="project-type">{project.type}</span><h3>{project.name}</h3><p>{project.description}</p><div className="project-bottom"><div className="project-tags">{project.tags.map((tag) => <span key={tag}>{tag}</span>)}</div><div className="project-links"><a href={project.url} target="_blank" rel="noreferrer" aria-label={`Open ${project.name}`}><ArrowUpRight size={14} /></a>{project.github && <a href={project.github} target="_blank" rel="noreferrer" aria-label={`${project.name} GitHub`}><Github size={14} /></a>}</div></div></div></article>)}</div></section>
